...
function RiskTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-white/90 border border-gray-200 rounded-lg px-3 py-2 text-xs shadow-lg">
      ...
      <p className={`font-bold font-mono ${val >= THRESHOLD ? 'text-red-500' : 'text-green-600'}`}>
...
function SHAPTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-white/90 border border-gray-200 rounded-lg px-3 py-2 text-xs shadow-lg">
      ...
      <p className={`font-mono ${val >= 0 ? 'text-red-500' : 'text-blue-500'}`}>
...
function AlertBanner({ result }) {
  ...
  return (
    <div className={`rounded-2xl p-4 border-2 flex items-center gap-4 backdrop-blur-sm ${
      isAlert
        ? 'bg-red-50/80 border-red-400 shadow-[0_0_20px_rgba(239,68,68,0.15)]'
        : 'bg-green-50/80 border-green-400'
    }`}>
      <span className="text-4xl">{isAlert ? '🚨' : '✅'}</span>
      <div>
        <p className={`font-bold text-lg ${isAlert ? 'text-red-600' : 'text-green-600'}`}>
        ...
        <p className="text-gray-600 text-sm mt-0.5">
          ...
          {isAlert && result.email_sent && (
            <span className="ml-3 text-green-600">· ✉️ Email alert sent</span>
          )}
        </p>
      </div>
    </div>
...
export default function PredictPage() {
  ...
  const handleDrop = (e) => {
    ...
    if (dropped?.name.endsWith('.csv') || dropped?.name.endsWith('.psv')) {
      setFile(dropped); setResult(null); setError(null); setSummary(null)
    } else {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setResult(data)
      if (data.peak_risk_score > 0) fetchSummary(data)
    } catch (e) {
      setError(e.message)
    } finally {
      setLoading(false)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-slate-100">
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        {/* Header */}
        <div>
          <h1 className="text-gray-800 text-2xl font-bold">CSV Patient Analysis</h1>
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all duration-200 backdrop-blur-sm ${
            dragging
        ...
        >
          ...
          />
          <p className="text-5xl mb-3">{file ? '📄' : '📂'}</p>
          {file ? (
            <>
            ...
            <>
              <p className="text-gray-600 font-medium text-lg">
                Drag & drop a patient CSV or PSV file
              </p>
        ...
        {file && !loading && (
          <button
            ...
            className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-xl transition-colors shadow-md"
          >
            🔬 Analyze Patient
          </button>
        )}
        ...
        {loading && (
          <div className="text-center py-10">
            <div className="inline-block w-10 h-10 border-4 border-blue-400 border-t-transparent rounded-full animate-spin mb-3" />
            <p className="text-gray-500">Analyzing patient vitals...</p>
          </div>
        )}

        {/* Error */}
        {error && (
          <div className="bg-red-50/80 border border-red-300 rounded-xl p-4 text-red-600 text-sm backdrop-blur-sm">
            ⚠️ {error}
          </div>
        )}
        ...
        {result && (
          <div className="space-y-5">
            ...
            {/* Risk Timeline */}
            <div className="bg-white/70 backdrop-blur-sm border border-gray-200 rounded-2xl p-5 shadow-sm">
              <h2 className="text-gray-700 font-semibold mb-4">📈 Risk Timeline</h2>
              {riskChartData.length < 2 ? (
                <p className="text-gray-400 text-sm text-center py-8">Not enough data points</p>
              ) : (
            ...
            {shapChartData.length > 0 && (
              <div className="bg-white/70 backdrop-blur-sm border border-gray-200 rounded-2xl p-5 shadow-sm">
                <h2 className="text-gray-700 font-semibold mb-1">🧠 SHAP Explainability</h2>
            ...
            {/* MedGemma Summary */}
            <div className="bg-blue-50/80 backdrop-blur-sm border border-blue-100 rounded-2xl p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-3">
                <h2 className="text-blue-700 font-semibold">🩺 Clinical Summary</h2>
              ...
              {summaryLoading && (
                <div className="space-y-2">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className={`h-4 bg-blue-100 rounded animate-pulse ${i === 3 ? 'w-2/3' : 'w-full'}`} />
                  ))}
                </div>
              )}
              ...
              {!summaryLoading && !summary && (
                <p className="text-gray-400 text-sm italic">
                  Summary unavailable — MedGemma may not be running.
                </p>
              )}
            </div>
            ...
            {result.alert_triggered && (
              <div className={`rounded-xl p-4 border text-sm backdrop-blur-sm ${
                result.email_sent
